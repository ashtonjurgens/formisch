...
> **Tip:** When you have access to a field store (from `useField` or `Field`), you can use `field.onInput(value)` to update the value directly. This is especially useful for custom inputs or component libraries that don't expose native HTML elements.
