...
Some component libraries (like MUI, Radix, or Headless UI) don't expose the underlying native HTML element, which means you cannot spread `field.props` onto them. For these cases, use `field.onChange` to set the value programmatically.
...
- **Component libraries** that wrap native elements without exposing them (MUI, Chakra UI, Radix, etc.)
- **Complex custom inputs** like date pickers, rich text editors, or color pickers
- **Third-party components** where you only have access to value/onChange props
